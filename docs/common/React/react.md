...
我们知道使用组件的时候，可以嵌套。要在自定义组件内使用嵌套结构，就需要使用 `props.children` 。在实际的工作当中，我们几乎每天都需要用这种方式来编写组件。
...
const map = fromJS(obj)      //将obj装成map
...
// [ 0, 1, 2, 3, 4, 5 ]
//展开运算符不能展开mutable对象，但是能展开List对象 
```
